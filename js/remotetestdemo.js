var remotedevicelist;

$(document).ready(function(){     

	    remotedevicelist = $('#remotedevlist').dataTable( {bFilter: false, bInfo: false, "bLengthChange": false,
		      "aoColumnDefs": [
...
var partnerEvent = new WebSocket('ws://35.164.253.193:4000/devctrl/devices/event');   // cs, bs, ack
var partnerLive = new WebSocket('ws://35.164.253.193:4000/devctrl/devices/live');   // pl
var nodeId;
...
// get device status
		...
		var resdt   = JSON.parse(obj);
		console.log("resdt.... "+resdt);
		var nId   = resdt.sensor;
		 //nodetype  = dt.sensor;
		 //payloadst   = dt.sensor;
		 console.log("data response "+nId);
		}
		});
