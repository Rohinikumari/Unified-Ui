...
*/
	...
	function processNodeCommands(inode, node) {
		for (var j = 0; j < node.np.length; j++) {
			...
			/*
			//start - Added for smart actuator
			if (Object.keys(node.np[j])[0] == 'voltage') {
				voltage = node.np[j][Object.keys(node.np[j])[0]];
				if ((voltage) && (voltage.length > 0)) {
					if (inode.voltage != voltage) {
						inode.lastStatus = voltage;
						//doFrameRefresh(inode);
					}
				}
			}
			if (Object.keys(node.np[j])[0] == 'current') {
				current = node.np[j][Object.keys(node.np[j])[0]];
				if ((current) && (current.length > 0)) {
					if (inode.current != current) {
						inode.current = current;
						//doFrameRefresh(inode);
					}
				}
			}
			//end - Added for smart actuator
			*/
