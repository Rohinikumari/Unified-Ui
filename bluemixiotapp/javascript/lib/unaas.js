var brokerConnect = (function() {
	...
	var onConnect = function(frame) {
		...
		for (i = 0; i < destEntityIdList.length; i++) {
			...
			wsClient.subscribe('iot-2/type/TestDeviceSwitch/id/' + destinationEntitiyId + '/evt/status/fmt/json');

			console.log('Subscribed to topic : iot-2/type/TestDeviceSwitch/id/' + destinationEntitiyId + '/evt/status/fmt/json');
		}
...
*/
	...
	function actionCommand(inode, value) {
		...
		var message = new Messaging.Message(msgJsonStr);
		//message.destinationName = 'Entity/' + inode.entityId + '/Node/' + inode.nodeId + '/CT';
		message.destinationName = 'iot-2/type/TestDeviceSwitch/id/' + inode.entityId + '/cmd/status/fmt/json'
		wsClient.send(message);
		console.log('MQTT:TOPIC:' + message.destinationName + ':MSG:' + msgJsonStr);
	}
